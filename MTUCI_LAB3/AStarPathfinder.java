 ...
 * Алгоритм реализован как статический метод, поскольку алгоритм поиска пути действительно не должен поддерживать
...
public class AStarPathfinder {
     ...
     * Если путь может быть найден, возвращается путевая точка шага <em> final </ em> в пути; эта точка может быть
     * использована для перехода назад к начальной точке.
    ...
    public static Waypoint computePath(Map2D map) {
        ...
        while (!foundPath && state.numOpenWaypoints() > 0) {
            ...
            // Наконец, переместите это место из «открытого» списка в «закрытый» список.
    ...
    /**
     * Этот статический вспомогательный метод берет путевую точку и генерирует все действительные «последующие шаги»
     * из этой путевой точки. Новые путевые точки добавляются в коллекцию «открытых путевых точек»
     * переданного объекта состояния A *.
     **/
    private static void takeNextStep(Waypoint currWP, AStarState state) {
        ...
        for (int y = loc.yCoord - 1; y <= loc.yCoord + 1; y++) {
            for (int x = loc.xCoord - 1; x <= loc.xCoord + 1; x++) {
                ...
                // Если «следующая локация» находится за пределами карты, пропустите ее.
                if (!map.contains(nextLoc))
                    continue;
                
                // Если следущее местоположение это такое же местоположение, то пропустите его.
                if (nextLoc == loc)
                    continue;
                
                // Если это местоположение уже находится в «закрытом» наборе, тогда переходите к следующему местоположению.
                ...
                // ОК, мы обманываем и используем оценку стоимости, чтобы вычислить фактическую стоимость из предыдущей ячейки.
                // Затем мы добавляем стоимость из ячейки карты, на которую мы вступаем, для включения барьеров и т. Д.
                ...
                // Если для этого местоположения уже есть путевая точка, новая путевая точка заменяет только старую путевую точку,
    ...
    /**
     * Оценивает стоимость прохода между двумя указанными местами.
