...
public class AStarApp {
    ...
    private void initGUI() {
        ...
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);  // Кнопка закрыть окно = выход из программы
        Container contentPane = frame.getContentPane();

        contentPane.setLayout(new BorderLayout());

        // Используйте GridBagLayout, потому что он фактически учитывает предпочтительный размер,
        // указанный компонентами, которые он размещает.
    ...
    private void findAndShowPath() {
        ...
        Map2D map = new Map2D(width, height);  // Ширина - 40, Высота - 30
        map.setStart(startLoc);  // Установить старт в точке, которую ввели ранее
        map.setFinish(finishLoc);  // Установить финиш в точке, которую задали ранее
    ...
    public static void main(String[] args) {
        AStarApp app = new AStarApp(40, 30);  // Тут мы указваем, какое будет поле (размер клеток - ширина / высота)
        app.start();
    }   
}
